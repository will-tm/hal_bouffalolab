...
/*
 * Structure containing the information about the PHY channel that was used for
 * this RX
 */
 ...
 * @param[in] config     Pointer to the PHY parameters (used radio paths, other
 * cfg)
 ****************************************************************************************
 */
 ...
 * This function returns the PHY version on two 32-bit words. The format of
 * these two words is PHY dependent.
 *
 ...
 * This function is called when the MAC stack needs to change the channel on
 * which the radio is locked. The function is blocking until the PLL has locked
 * on the new channel frequency.
 *
 ...
 *                          =prim20_freq for a 20MHz channel. Center or primary
 * 80MHz channel if 80+80 type
 * @param[in] center2_freq  Center frequency of secondary 80MHz channel if 80+80
 * @param[in] index         Index of the RF for which the channel has to be set
 * (@ref PHY_PRIM operating (primary RF)), 1: secondary RF (@ref PHY_SEC used
 * for additional radar detection). This parameter is reserved if no secondary
 * RF is available in the system
 ****************************************************************************************
 */
 ...
 * This function is called when the MAC stack needs to get the information on
 * the channel on which the radio is currently locked.
 *
 * @param[out] info        Pointer on the phy_channel_info structure used to
 * return the information
 * @param[in] index        Index of the RF for which the channel has to be
 * retrieved
 ...
 * This function is called when a Modem interrupt is asserted to the interrupt
 * controller
 ****************************************************************************************
 */
 ...
 * This function is called when a RC interrupt is asserted to the interrupt
 * controller
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Read a radar pulse from the FIFO, and writes it to the buffer passed
 * as parameter
 *
...
/**
 ****************************************************************************************
 * @brief Set the MU-MIMO RX group information as received in the VHT Group ID
 * action frame
 *
 * @param[in] membership_addr  Address of the membership status in the received
 * frame
 * @param[in] userpos_addr     Address of the user position array in the
 * received frame
 ****************************************************************************************
 */
 ...
 *
 * @return Number of Spatial Streams supported by the PHY (start from 0, 0 is
 * for 1 SS)
 ****************************************************************************************
 */
 ...
 * Return the idx to use (set in policy table), to configure a specific TX
 * power. If the TX power requested is not possible, it should return idx to get
 * the maximum TX power that do not exceed the request. In this case, power
 * parameter should be updated to the actual TX power set.
 *
 * @param[in,out] power TX power (in dBm) to configure for the radio, (updated
 * with the actual power set if request is not possible)
 * @param[out]    idx   Idx to use in policy table to configure a tx power of
 * \<gain\>
 ****************************************************************************************
 */
void phy_get_rf_gain_idx(int8_t *power, uint8_t *idx);
void phy_get_trpc_idx(uint8_t formatmod, uint8_t mcs, int8_t power,
		      uint8_t *idx);
...
/**
 ****************************************************************************************
 * @brief Return the frequency of the MAC core clock in case it would have been
 * adjusted depending on the PHY properties.
 *
...
/**
 ****************************************************************************************
 * @brief Read the state of all PHY diagnostic ports and put it in the debug
 * dump info structure
 *
