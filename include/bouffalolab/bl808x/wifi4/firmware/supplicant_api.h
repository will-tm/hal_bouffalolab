...
typedef struct {
	bool capable;  /**< Advertizes support for Protected Management Frame.
			  Device will prefer to connect in PMF mode if other
			  device also advertizes PMF capability. */
	bool required; /**< Advertizes that Protected Management Frame is
			  required. Device will not associate to non-PMF capable
			  devices. */
} wifi_pmf_config_t;
...
typedef struct {
	...
	uint16_t proto;       /* sec_proto_t — firmware compiled with -fshort-enums */
	uint16_t key_mgmt;
	uint8_t pairwise_cipher;
	uint8_t group_cipher;
	char passphrase[64 + 1];
	bool pmf_required;
	uint8_t mgmt_group_cipher; /* should always be WPA_CIPHER_AES_128_CMAC */
				   /* if PMFR=1 */
} wifi_connect_parm_t;
...
struct wpa_funcs {
	...
	bool (*wpa_ap_join)(void **sm, uint8_t *mac, uint8_t *wpa_ie,
			    uint8_t wpa_ie_len);
	...
	uint8_t *(*wpa3_build_sae_msg)(uint8_t *bssid, uint8_t *mac,
				       uint8_t *passphrase,
