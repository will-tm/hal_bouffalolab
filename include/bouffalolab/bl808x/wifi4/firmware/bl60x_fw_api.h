...
typedef enum wifi_fw_task_id {
	...
	/*
	 * This is used to define the last task that is running on the EMB
	 * processor
	 */
...
typedef enum wifi_fw_event_id {
	...
	MM_SET_IDLE_CFM,
	/*
	 * Indication of the primary TBTT to the upper MAC. Upon the reception
	 * of this message the upper MAC has to push the beacon(s) to the
	 * beacon transmission queue.
	 */
	MM_PRIMARY_TBTT_IND,
	/*
	 * Indication of the secondary TBTT to the upper MAC. Upon the
	 * reception of this message the upper MAC has to push the beacon(s) to
	 * the beacon transmission queue.
	 */
	MM_SECONDARY_TBTT_IND,
	/*
	 * Request to the LMAC to trigger the embedded logic analyzer and
	 * forward the debug dump.
	 */
	MM_DENOISE_REQ,
	/* Set Power Save mode */
	MM_SET_PS_MODE_REQ,
	...
	MM_PS_CHANGE_IND,
	/*
	 * Indication that some buffered traffic should be sent to the peer
	 * device
	 */
	MM_TRAFFIC_REQ_IND,
	...
	/*
	 * Section of internal MM messages. No MM API messages should be defined
	 * below this point
	 */
	...
	/*
	 * Section of internal SCAN messages. No SCAN API messages should be
	...
	/*
	 * Section of internal ME messages. No ME API messages should be defined
	 * below this point
	 */
	/*
	 * Internal request to indicate that a VIF needs to get the HW going to
	 * ACTIVE or IDLE
	 */
	ME_SET_ACTIVE_REQ,
	...
	ME_SET_ACTIVE_CFM,
	/*
	 * Internal request to indicate that a VIF desires to
	 * de-activate/activate the Power-Save mode
	 */
	ME_SET_PS_DISABLE_REQ,
	/*
	 * Confirmation that the PS state de-activate/activate has been
	 * executed
	 */
...
void bl60x_fw_xtal_capcode_set(uint8_t cap_in, uint8_t cap_out, uint8_t enable,
			       uint8_t cap_in_max, uint8_t cap_out_max);
...
int bl60x_fw_password_hash(char *password, char *ssid, int ssidlength,
			   char *output);
void bl60x_fw_rf_table_set(uint32_t *channel_div_table_in,
			   uint16_t *channel_cnt_table_in,
...
int bl60x_edca_get(int ac, uint8_t *aifs, uint8_t *cwmin, uint8_t *cwmax,
		   uint16_t *txop);
