...
enum ap_info_type {
	 ...
	 * when reconnecting. If the time_to_live field is not 0, the
	 * information will not be used after time_to_live times.
	 */
...
struct ap_info {
	...
	/* when type field is AP_INFO_TYPE_SUGGEST, this field indicates the
...
struct ap_connect_adv {
	...
	/* MISC flags */
	/* XXX following flag values and connection flags defined in mac.h
	 * should be identical */
...
typedef void (*scan_item_cb_t)(wifi_mgmr_ap_item_t *env, uint32_t *param1,
...
enum WIFI_STATE_ENUM_LIST {
	BL_WIFI_STATE_UNKNOWN = 0x00,
	BL_WIFI_STATE_IDLE = 0x01,
	BL_WIFI_STATE_CONNECTING = 0x02,
	BL_WIFI_STATE_CONNECTED_IP_GETTING = 0x03,
	BL_WIFI_STATE_CONNECTED_IP_GOT = 0x04,
	BL_WIFI_STATE_DISCONNECT = 0x05,
	BL_WIFI_STATE_WITH_AP_IDLE = 0x11,
	BL_WIFI_STATE_WITH_AP_CONNECTING = 0x12,
	BL_WIFI_STATE_WITH_AP_CONNECTED_IP_GETTING = 0x13,
	BL_WIFI_STATE_WITH_AP_CONNECTED_IP_GOT = 0x14,
	BL_WIFI_STATE_WITH_AP_DISCONNECT = 0x15,
	BL_WIFI_STATE_IFDOWN = 0x06,
	BL_WIFI_STATE_SNIFFER = 0x07,
	BL_WIFI_STATE_PSK_ERROR = 0x08,
	BL_WIFI_STATE_NO_AP_FOUND = 0x09,
};
#define BL_WIFI_STATE_AP_IS_ENABLED(status) ((status) & 0x10)
...
int wifi_mgmr_sta_connect_mid(wifi_interface_t *wifi_interface, char *ssid, char *psk, char *pmk,
			      uint8_t *mac, uint8_t band, uint8_t chan_id, uint8_t use_dhcp,
			      uint32_t flags);
int wifi_mgmr_sta_connect(wifi_interface_t *wifi_interface, char *ssid, char *psk, char *pmk,
			  uint8_t *mac, uint8_t band, uint8_t chan_id);
...
void wifi_mgmr_sta_psk_set(char *psk)
	__attribute__((deprecated("use wifi_mgmr_sta_passphr_set instead")));
...
int wifi_mgmr_ap_start(wifi_interface_t *interface, char *ssid, int hidden_ssid, char *passwd,
		       int channel);
int wifi_mgmr_ap_start_adv(wifi_interface_t *interface, char *ssid, int hidden_ssid, char *passwd,
			   int channel, uint8_t use_dhcp);
int wifi_mgmr_ap_start_atcmd(wifi_interface_t *interface, char *ssid, int hidden_ssid, char *passwd,
			     int channel, int max_sta_supported);
...
/* Advanced exposes low level representation of data(struct pbuf for most
 * systems). */
...
int wifi_mgmr_scan_adv(void *data, scan_complete_cb_t cb, uint16_t *channels, uint16_t channel_num,
		       const uint8_t bssid[6], const char *ssid, uint8_t scan_mode,
