 ...
 *
 * Based on Bouffalo Lab SDK bl_os_adapter.h (Apache-2.0)
 */
...
struct bl_ops_funcs {
	...
	uint32_t (*_event_group_wait)(BL_EventGroup_t event, uint32_t bits_to_wait_for,
	...
	int (*_task_create)(const char *name, void *entry, uint32_t stack_depth, void *param,
	...
	int (*_queue_send_wait)(BL_MessageQueue_t queue, void *item, uint32_t len, uint32_t ticks,
				int prio);
